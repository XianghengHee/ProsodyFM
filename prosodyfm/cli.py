...
from prosodyfm.models.prosodyfm import MatchaTTS
from prosodyfm.text import sequence_to_text, text_to_sequence
from prosodyfm.utils.utils import assert_model_downloaded, get_user_data_dir, intersperse
...
def process_text(i: int, text: str, device: torch.device):
    ...
    sequence, intonation_sequence = text_to_sequence(text, ['english_cleaners2'])
    x = torch.tensor(intersperse(sequence, 0),dtype=torch.long, device=device)[None]
    x_intonation = torch.tensor(intersperse(intonation_sequence, 3),dtype=torch.long, device=device)[None]
    x_lengths = torch.tensor([x.shape[-1]], dtype=torch.long, device=device)
    x_phones = sequence_to_text(x.squeeze(0).tolist())
    print(f"[{i}] - Phonetised text: {x_phones[1::2]}")

    return {"x_orig": text, "x": x, "x_lengths": x_lengths, "x_phones": x_phones, 'x_intonation': x_intonation}
...
def batched_collate_fn(batch):
    ...
    for b in batch:
        ...
        x_intonation.append(b['x_intonation'].squeeze(0))


    x = torch.nn.utils.rnn.pad_sequence(x, batch_first=True)
    x_intonation = torch.nn.utils.rnn.pad_sequence(x_intonation, batch_first=True)
    x_lengths = torch.concat(x_lengths, dim=0)
    return {"x": x, "x_lengths": x_lengths, "x_intonation": x_intonation}
...
def batched_synthesis(args, device, model, vocoder, denoiser, texts, spk):
    ...
    for i, batch in enumerate(dataloader):
        ...
        output = model.synthesise(
            ...
            spks=spk,
            x_intonation = batch['x_intonation'].to(device),
...
def unbatched_synthesis(args, device, model, vocoder, denoiser, texts, spk):
    ...
    for i, text in enumerate(texts):
        ...
        output = model.synthesise(
            ...
            temperature=args.temperature,
            spks=spk,
            x_intonation = text_processed['x_intonation'],
            length_scale=args.speaking_rate,
        )
        output["waveform"] = to_waveform(output["mel"], vocoder, denoiser)
        # RTF with HiFiGAN
